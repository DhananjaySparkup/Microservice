...
exports.assignService = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error assigning service', error: err.message });
  }
};
...
exports.calculateCharge = async (req, res) => {
  ...
  try {
    ...
    if (!charge) return res.status(400).json({ message: 'No matching slab for amount' });

    res.status(200).json({ serviceCharge: charge });
  } catch (err) {
    res.status(500).json({ message: 'Error calculating charge', error: err.message });
  }
};
